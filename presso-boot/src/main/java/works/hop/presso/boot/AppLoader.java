 ...
 * Expects the application jar file to contain a manifest, and a Main-Class attribute in that manifest.
 */
...
public class AppLoader {
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException, NoSuchMethodException, InvocationTargetException {
        ...
        if (rootDir.isDirectory()) {
            File[] files = Objects.requireNonNull(rootDir.listFiles(pathname -> pathname.isFile() && pathname.getPath().endsWith(".jar")));
            if (files.length == 0) {
